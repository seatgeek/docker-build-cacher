                ...
                Build -> do
                    ...
                    buildOPtions <-
                        do opts <- need "DOCKER_BUILD_OPTIONS"
                           if noBuildCache
                               then return $
                                    Just (opts & fromMaybe "" & (<> " --no-cache") & Docker.Cacher.BuildOptions) -- Append the docker --no-cache option
                               else return (fmap Docker.Cacher.BuildOptions opts)
                    if noCacheStages
                        then sh (Docker.Cacher.build app name buildOPtions ast)
                        else sh (Docker.Cacher.buildFromCache app branch fallbackBranch name buildOPtions ast)
